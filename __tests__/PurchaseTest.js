...
const runExceptionByPurchaseInput = async (inputs) => {
  ...
  // then
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.priceUnit));
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.notNumber));
...
const runExceptionByWinningNumInput = async (inputs) => {
  ...
  // then
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.notLength));
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.notComma));
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.notRange));
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.repeatNum));
...
const runExceptionByBonusNumInput = async (input) => {
  ...
  // then
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.notBonusRange));
  expect(logSpy).toHaveBeenCalledWith(expect.stringContaining(MESSAGE.error.repeatBonusNum));
